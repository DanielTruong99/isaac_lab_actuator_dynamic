    ...
    while simulation_app.is_running():
        ...
        if state == "WALK":
            if signal == "entry":
                ...
                counter_5s = 5.0 / dt
                env.unwrapped.action_manager._terms["joint_pos"].alpha = 0.7  # fc = 3.5 hz
                # counter = 0

            if signal == "50hz_timeout":
                if counter > counter_5s:
                    vel_cmds = torch.tensor([0.35, 0.0, 0.0], device=env.unwrapped.device)
                ...
                with torch.inference_mode():
                    ...
                    #     joint_vels[counter] = (action - prev_action)
                    # else:
                    #     joint_vels = joint_vels.cpu().numpy()
                    #     plt.figure()
                    #     for j in range(asset.num_joints):
                    #         plt.plot(joint_vels[:,0,j], label=f'Joint {j}')
