...
def main(env_cfg: ManagerBasedRLEnvCfg | DirectRLEnvCfg, agent_cfg: RslRlBaseRunnerCfg):
    ...
    while simulation_app.is_running():
        ...
        # get joint commands
        joint_pos_recorded = env.unwrapped.recorded_joint_pos.cpu().numpy().tolist()
        # joint_pos_recorded = env.unwrapped.recorded_joint_pos_cmds.cpu().numpy().tolist()
        # joint_pos_recorded[0][3] -= 0.07 # add offset to L_calf_joint
        joint_pos_recorded_data += joint_pos_recorded
        ...
        if dones.any():
            ...
            for i in range(joint_data.shape[1]):
                plt.figure()
                plt.plot(time_data, joint_data[:, i], label='Joint Position Simulation')
                plt.plot(time_data, joint_pos_recorded_data[:, i], label='Joint Position Real', linestyle='--')
                plt.xlabel('Time (s)')
                plt.ylabel('Position (rad)')
                plt.title(f'Joint {i} Position Sim vs Real')
                plt.legend()
                plt.grid()
