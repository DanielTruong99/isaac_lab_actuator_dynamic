...
class ActuatorDynamicEnv(DirectRLEnv):
    ...
    def __init__(self, cfg: ActuatorDynamicEnvCfg, render_mode: str | None = None, **kwargs):
        ...
        #! Need to be changed
        self.key_joint_indexes = self.robot.find_joints(["L_hip2_joint"])[0]
        self.ref_body_index = self.robot.data.body_names.index('base')
        self.key_body_indexes = [self.robot.data.body_names.index(name) for name in ["L_hip2"]]
        self.motion_dof_indexes = self._motion_loader.get_dof_index(["L_hip2_joint"])
