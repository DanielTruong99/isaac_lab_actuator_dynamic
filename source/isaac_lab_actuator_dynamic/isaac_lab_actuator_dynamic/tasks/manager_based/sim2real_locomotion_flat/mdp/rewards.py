...
def torque_rate_l2(env: ManagerBasedRLEnv) -> torch.Tensor:
...
def hip2_joint_deviation(env: WalkingRobotEnv, asset_cfg: SceneEntityCfg = SceneEntityCfg("robot")) -> torch.Tensor:
    """
    Calculate the hip 2 joint deviation for a walking robot environment.
    ...
    asset = env.scene[asset_cfg.name]
    # compute out of limits constraints
    angle = asset.data.joint_pos[:, asset_cfg.joint_ids[0]:asset_cfg.joint_ids[0]+1] + asset.data.joint_pos[:, asset_cfg.joint_ids[1]:asset_cfg.joint_ids[1]+1]
    return torch.sum(torch.abs(angle), dim=1) * is_vcmd_gt
...
def knee_joint_torques_standstill(env: ManagerBasedRLEnv, asset_cfg: SceneEntityCfg = SceneEntityCfg("robot")) -> torch.Tensor:
    ...
    # extract the used quantities (to enable type-hinting)
    standing_env_ids = env.command_manager._terms["base_velocity"].is_standing_env.nonzero(as_tuple=False).flatten()
    asset = env.scene[asset_cfg.name]
    applied_torque_standing = torch.zeros((env.num_envs, asset.num_joints), device=env.device)
    applied_torque_standing[standing_env_ids, :] = asset.data.applied_torque[standing_env_ids, :]
    return torch.sum(torch.square(applied_torque_standing[:, asset_cfg.joint_ids]), dim=1)

def knee_joint_powers_l1(env: ManagerBasedRLEnv, asset_cfg: SceneEntityCfg = SceneEntityCfg("robot")) -> torch.Tensor:
    """Penalize joint powers on the articulation using L1-kernel"""
...
class TorqueRatePenalty(ManagerTermBase):
    ...
    def __call__(self, env: ManagerBasedRLEnv) -> torch.Tensor:
        ...
        current_torque = env.scene["robot"].data.applied_torque.clone()
        ...
        startup_env_mask = env.episode_length_buf < 1
