...
class MotionCommand(CommandTerm):
    ...
    def _uniform_sampling(self, env_ids: Sequence[int]):
        ...
        # Uniform probability for all bins
        sampling_probabilities = torch.ones(num_bins, device=self.device) / float(num_bins)

        # Sample bins uniformly
        sampled_bins = torch.multinomial(sampling_probabilities, num_envs, replacement=True)

        # Compute sampled timesteps
        self.time_steps[env_ids] = (
            (sampled_bins + torch.rand(num_envs, device=self.device))
            / num_bins
        ...
        H_norm = H / math.log(num_bins)
        pmax, imax = sampling_probabilities.max(dim=0)

        self.metrics["sampling_entropy"][:] = H_norm           # should be ≈ 1.0
        self.metrics["sampling_top1_prob"][:] = pmax            # = 1 / num_bins
        self.metrics["sampling_top1_bin"][:] = imax.float() / num_bins  # usually 0 since uniform
