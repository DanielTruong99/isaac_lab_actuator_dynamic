...
class MotionLoader:
    def __init__(self, motion_files: str, num_envs, body_indexes: Sequence[int], device: str = "cpu"):
        ...
        for index, motion_file in enumerate(motion_files):
            ...
            body_pos_w = data["xpos"][:, [1, *list(range(11, 11+10))], :]
            # body_pos_w[:, :, 2] -= 0.08  # Adjust height if necessary
            body_pos_w_list.append(body_pos_w)

            body_quat_w = data["xquat"][:, [1, *list(range(11, 11+10))], :]
            body_quat_w_list.append(body_quat_w)

            body_lin_vel_w =  data["cvel"][:, [1, *list(range(11, 11+10))], :3]
            body_lin_vel_w_list.append(body_lin_vel_w)

            body_ang_vel_w = data["cvel"][:, [1, *list(range(11, 11+10))], 3:]
            body_ang_vel_w_list.append(body_ang_vel_w)
        
        self.time_step_total = torch.tensor(time_step_total_list, dtype=torch.long, device=device)
        ...
        self.fps = data["frequency"]

        self.num_envs = num_envs
        self.motion_ids = torch.multinomial(input=torch.ones(len(motion_files), device=device), num_samples=num_envs, replacement=True)
        time_step_cumsum = torch.cumsum(self.time_step_total, dim=0)
        self.end_indices = time_step_cumsum[self.motion_ids]

        time_step_rolled = self.time_step_total.roll(shifts=1)
        time_step_rolled[0] = 0
        self.start_indices = torch.cumsum(time_step_rolled, dim=0)[self.motion_ids] 
        self.time_step_total = self.time_step_total[self.motion_ids]
