...
def position_orientation_bonus(
    ...
    goal = quat_distance < 0.1
    goal = torch.logical_and(goal, distance < 1e-3)
    return goal.float()

def modified_joint_torques_l2(env: ManagerBasedRLEnv, asset_cfg: SceneEntityCfg, command_name: str) -> torch.Tensor:
    ...
    curr_quat_w = asset.data.body_state_w[:, asset_cfg.body_ids[0], 3:7]  # type: ignore
    quat_distance = quat_error_magnitude(curr_quat_w, des_quat_w)
    quat_goal = quat_distance < 0.1

    reward = torch.sum(torch.square(asset.data.applied_torque[:, asset_cfg.joint_ids]), dim=1) * quat_goal.float()
    return reward

def position_command_error_multi_scale(env: ManagerBasedRLEnv, std: float, command_name: str, asset_cfg: SceneEntityCfg
) -> torch.Tensor:
    ...
    reward = (
        - 1.0 * distance
        + 1.0 / (torch.exp(-50 * distance) + torch.exp(50 * distance) + 1e-6)
        + 1.0 / (torch.exp(-300 * distance) + torch.exp(300 * distance) + 1e-6)
    )
    return reward
